...
class KeychainShimDB: NSObject {
    public static let shared = KeychainShimDB()
    ...
    func query(_ attributes: NSDictionary) -> [NSMutableDictionary]? {
        guard let tableName = attributes[kSecClass] as? String,
              let primaryColumns = VaultDBConstants.primaries[tableName as CFString] else {
            return nil
        }
        ...
        guard usingDB({ sqlite3DB in
            ...
            guard sqlite3_prepare(sqlite3DB, selectQuery, -1, &stmt, nil) == SQLITE_OK,
                  let stmt = stmt else {
                KeychainShim.debugLogger("Failed query \(selectQuery)")
                KeychainShim.debugLogger("Failed to query to db table: \(String(cString: sqlite3_errmsg(sqlite3DB)))")
                return false
            }
    ...
    func insert(_ attributes: NSDictionary) -> NSMutableDictionary? {
        guard let tableName = attributes[kSecClass] as? String,
              let primaryColumns = VaultDBConstants.primaries[tableName as CFString],
              let secondaryColumns = VaultDBConstants.attributes[tableName as CFString] else {
            return nil
        }
        ...
        columnsQuery.append(contentsOf: VaultDBConstants.values.compactMap({
            attributes[$0] != nil ? "\($0)" : nil
        }))
        ...
        guard usingDB({ sqlite3DB in
            ...
            guard sqlite3_prepare(sqlite3DB, insertQuery, -1, &stmt, nil) == SQLITE_OK,
                ...
                KeychainShim.debugLogger("Failed query \(insertQuery)")
                KeychainShim.debugLogger("Failed to make query: \(errorMessage)")
                return false
            }
            ...
            where !encodeData(stmt: stmt, index: Int32(index + 1), value: value) {
                ...
                KeychainShim.debugLogger("Failed to insert into db: \(erorrMessage)")
                return false
            }
    ...
    func update(_ attributes: NSDictionary) -> Bool {
        ...
        guard usingDB({ sqlite3DB in
            ...
            guard sqlite3_prepare(sqlite3DB, updateQuery, -1, &stmt, nil) == SQLITE_OK,
                  let stmt = stmt else {
                let errorMessage = String(cString: sqlite3_errmsg(sqlite3DB))
                KeychainShim.debugLogger("Failed query \(updateQuery)")
                KeychainShim.debugLogger("Failed to make query: \(errorMessage)")
                return false
            }
            ...
            where !encodeData(stmt: stmt, index: Int32(index + 1), value: value) {
                let errorMessage = String(cString: sqlite3_errmsg(sqlite3DB))
                KeychainShim.debugLogger("Failed to update into db: \(errorMessage)")
                return false
            }
    ...
    func delete(_ attributes: NSDictionary) -> Bool {
        ...
        guard usingDB({ sqlite3DB in
            ...
            guard sqlite3_prepare(sqlite3DB, deleteQuery, -1, &stmt, nil) == SQLITE_OK,
                  let stmt = stmt else {
                let errorMessage = String(cString: sqlite3_errmsg(sqlite3DB))
                KeychainShim.debugLogger("Failed query \(deleteQuery)")
                KeychainShim.debugLogger("Failed to delte items from db table: \(errorMessage)")
                return false
            }
    ...
    private func structDB(_ sqlite3DB: OpaquePointer) -> Bool {
        for (key, value) in VaultDBConstants.primaries {
            var columns: [CFString] = value
            columns.append(contentsOf: VaultDBConstants.attributes[key]!)
            columns.append(contentsOf: VaultDBConstants.values)
            ...
            guard sqlite3_exec(sqlite3DB, createTableQuery, nil, nil, nil) == SQLITE_OK else {
                let errorMessage = String(cString: sqlite3_errmsg(sqlite3DB))
                KeychainShim.debugLogger("Failed query \(createTableQuery)")
                KeychainShim.debugLogger("Failed to create db table: \(errorMessage)")
                return false
            }
        }
    ...
    private func connectToDB() -> OpaquePointer? {
        ...
        let keychainDB = URL(fileURLWithPath: "/Users/\(NSUserName())/Library/Containers/be.ophanim.Ophanim")
            .appendingPathComponent("ChainGuard")
        ...
        guard sqlite3_open(keychainDB.path, &sqlite3DB) == SQLITE_OK,
            ...
            KeychainShim.debugLogger("Failed to connect to DB")
            return nil
        }
    ...
    private func disconnectFromDB(_ sqlite3DB: OpaquePointer?) -> Bool {
        guard sqlite3_close(sqlite3DB) == SQLITE_OK else {
            KeychainShim.debugLogger("Failed to disconnect from DB")
            return false
        }
    ...
    private func encodeData(stmt: OpaquePointer, index: Int32, value: CFTypeRef) -> Bool {
        ...
        default:
            KeychainShim.debugLogger("Cannot encode this data type: \(CFGetTypeID(value))")
            result = sqlite3_bind_null(stmt, index)
        }
    ...
    private func decodeData(stmt: OpaquePointer, index: Int32) -> CFTypeRef? {
        ...
        case SQLITE_NULL:
            return nil
        default:
            KeychainShim.debugLogger("Cannot decode this data \(sqlite3_column_type(stmt, index))")
            return nil
        }
    }
}
