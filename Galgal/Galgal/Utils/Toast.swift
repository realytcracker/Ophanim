...
class Toast {
    ...
    static func syncUserDefaults() -> Float {
        let persistenceKeyname = "galgalKeymappingDisabledAt"
        ...
        let center = NotificationCenter.default
        token2 = center.addObserver(forName: NSNotification.Name.galgalCursorWillShow,
    ...
    public static func initialize() {
        ...
        Toast.showHint(title: NSLocalizedString("hint.mouseMapping.title",
                       ...
                       timeout: 10,
                       notification: NSNotification.Name.galgalCursorWillHide)
        let center = NotificationCenter.default
        var token: NSObjectProtocol?
        token = center.addObserver(forName: NSNotification.Name.galgalCursorWillHide,
                                   object: nil, queue: OperationQueue.main) { _ in
            center.removeObserver(token!)
            Toast.showHint(title: NSLocalizedString("hint.showCursor.title",
                           ...
                           timeout: 10,
                           notification: NSNotification.Name.galgalCursorWillShow)
        }
    }
