    ...
    func beginTouch(_ point: CGPoint) {
        if mode == .FIXED {
            ...
            Toucher.touchcam(point: self.center, phase: UITouch.Phase.began, tid: &id,
                             actionName: "ControllerJoystick", keyName: key)
            GalgalInput.touchQueue.asyncAfter(deadline: .now() + 0.04, qos: .userInitiated) {
                if self.id == nil {
                    return
                }
...
class SwipeAction: Action {
    ...
    private var id: Int?
    let timer = DispatchSource.makeTimerSource(flags: [], queue: GalgalInput.touchQueue)
    ...
    func delay(_ delay: Double, closure: @escaping () -> Void) {
        ...
        GalgalInput.touchQueue.asyncAfter(deadline: when, execute: closure)
    }
    ...
    func invalidate() {
        GalgalInput.touchQueue.async(execute: self.doLiftOff)
    }
}
