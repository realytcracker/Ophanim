...
- (CGRect) hook_frameDefault {
    return [GalgalScreen frameDefault:[self hook_frameDefault]];
}

- (CGRect) hook_boundsDefault {
    return [GalgalScreen boundsDefault:[self hook_boundsDefault]];
}

- (CGRect) hook_nativeBoundsDefault {
    return [GalgalScreen nativeBoundsDefault:[self hook_nativeBoundsDefault]];
}

- (CGSize) hook_sizeDelfault {
    return [GalgalScreen sizeAspectRatioDefault:[self hook_sizeDelfault]];
}


- (CGRect) hook_frame {
    return [GalgalScreen frame:[self hook_frame]];
}

- (CGRect) hook_bounds {
    return [GalgalScreen bounds:[self hook_bounds]];
}

- (CGRect) hook_nativeBounds {
    return [GalgalScreen nativeBounds:[self hook_nativeBounds]];
}

- (CGSize) hook_size {
    return [GalgalScreen sizeAspectRatio:[self hook_size]];
}
...
- (double) hook_nativeScale {
    return [[AppConfig shared] customScaler];
}

- (double) hook_scale {
    // Return rounded value of [[AppConfig shared] customScaler]
    // Even though it is a double return, this will only accept .0 value or apps will crash
    return round([[AppConfig shared] customScaler]);
}
...
- (CGRect) hook_boundsResizable {
    return [GalgalScreen boundsResizable:[self hook_boundsResizable]];
}
...
/*
 ...
 is called. This way we can guarantee the hooks will be applied before [Ophanim launch] is called (in GalgalLoader.m).
...
+ (void)load {
    ...
    if(@available(iOS 16.3, *)) {
        if ([[AppConfig shared] resizableWindow]) {
        ...
        else if ([[AppConfig shared] adaptiveDisplay]) {
            // This is an experimental fix
            if ([[AppConfig shared] inverseScreenValues]) {
            ...
            if ([[AppConfig shared] windowFixMethod] == 1) {
                // do nothing:tm:
            }
            else {
                CGFloat newValueW = (CGFloat) [self get_default_width];
                [[AppConfig shared] setValue:@(newValueW) forKey:@"windowSizeWidth"];
                
                CGFloat newValueH = (CGFloat)[self get_default_height];
                [[AppConfig shared] setValue:@(newValueH) forKey:@"windowSizeHeight"];
                if (![[AppConfig shared] inverseScreenValues]) {
                    if(@available(iOS 17.1, *))
                        [objc_getClass("FBSSceneSettingsCore") swizzleExchangeMethod:@selector(frame) withMethod:@selector(hook_frameDefault)];
                    else
        ...
        if ([[AppConfig shared] adaptiveDisplay]) {
                if(@available(iOS 17.1, *))
                    [objc_getClass("FBSSceneSettingsCore") swizzleExchangeMethod:@selector(frame) withMethod:@selector(hook_frame)];
                else
                    [objc_getClass("FBSSceneSettings") swizzleInstanceMethod:@selector(frame) withMethod:@selector(hook_frame)];
    ...
    if ([[AppConfig shared] checkMicPermissionSync]) {
