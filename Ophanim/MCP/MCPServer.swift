...
final class MCPServer {
    ...
    private func decode<T: Decodable>(_ obj: Any, as type: T.Type = T.self, label: String) throws -> T {
    ...
    private func runTool(_ name: String, _ args: [String: Any]) throws -> String {
        ...
        case "set_objc_hooks":
            ...
            let hooks: [OPObjCHook] = try decode(hooksArg, label: "hooks")
        ...
        case "set_swift_hooks":
            guard let bid = args["bundleID"] as? String, !bid.isEmpty else { throw bail("bundleID is required") }
            guard let hooksArg = args["hooks"] else { throw bail("hooks array is required") }
            let hooks: [OPSwiftHook] = try decode(hooksArg, label: "hooks")
        ...
        case "set_inline_hooks":
            guard let bid = args["bundleID"] as? String, !bid.isEmpty else { throw bail("bundleID is required") }
            guard let hooksArg = args["hooks"] else { throw bail("hooks array is required") }
            let hooks: [OPInlineHook] = try decode(hooksArg, label: "hooks")
        ...
        case "set_rules":
            ...
            let rules: [OPRule] = try decode(rulesArg, label: "rules")
