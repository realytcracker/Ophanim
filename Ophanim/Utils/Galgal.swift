...
class Galgal {
    ...
    static func installOnSystem() {
        Task(priority: .background) {
            do {
                ...
                // Check if a version of Galgal is already installed, if so remove it
                FileManager.default.delete(at: URL(fileURLWithPath: galgalFramework.path))

                // Install version of Galgal bundled with Ophanim
                Log.shared.log("Copying Galgal to Frameworks")
                if FileManager.default.fileExists(atPath: galgalFramework.path) {
                    try FileManager.default.removeItem(at: galgalFramework)
                }
                try FileManager.default.copyItem(at: bundledGalgalFramework, to: galgalFramework)
    ...
    static func installAgentOnSystem() {
        ...
        do {
            ...
            try FileManager.default.copyItem(at: bundledAgent, to: agentPath)
        } catch {
            Log.shared.error(error)
        }
    }
